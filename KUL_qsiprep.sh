...
if [ "$#" -lt 1 ]; then
	...
	while getopts "w:p:n:m:t:gv" OPT; do
...
if [ $sdc -eq 1 ]; then
	...
	synb0_scratch="${cwd}/synb0_work_${participant}"
	...
	if [ ! -f $test_file ];then
		...
		if [ $synb0out -eq 1 ];then 
			...
			# make a json
			json_file=${synb0_scratch}/sub-${participant}_dir-${dir_epi}_epi.json
			echo "{" > $json_file
			...
			mkdir -p $qsi_data/sub-${participant}/fmap
			cp $json_file $qsi_data/sub-${participant}/fmap
			cp $synb0_scratch/OUTPUTS/b0_u.nii.gz $qsi_data/sub-${participant}/fmap/sub-${participant}_dir-${dir_epi}_epi.nii.gz
