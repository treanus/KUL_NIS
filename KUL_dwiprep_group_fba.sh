...
# Changes made by AR and AT (09/03/2023):
# 1- Fixed the msmt fod calculation to also do gm.mg, and removed the nogm suffix from the wmfod and csf
# 2- Made the warp2metric and fixelcorrespondence for_each loops use 1 threads, to avoid errors in parallel work
# 3- Added if statements to make s=subname if no session is defined, or s=subname_ses if session is defined
...
if [ ! -f ../fod_estimation.done ]; then
    ...
    if [ "$algo" = "ssst" ]; then 
        ...
        -mask IN/dwi_preproced${suffix}_mask.mif -force -nthreads 2
...
if [ ! -f ../fod_reg2template.done ]; then
    ...
    for_each -info -force -nthreads ${ncpu_foreach} * : mrregister IN/wmfod_norm.mif -mask1 IN/dwi_preproced${suffix}_mask.mif \
...
if [ ! -f ../template/template_mask.mif ]; then
    ...
    if [ "$algo" = "ssst" ]; then 
        ...
        for_each -info -force -nthreads ${ncpu_foreach} * : mrtransform IN/dwi_preproced${suffix}_normalised_mask.mif -warp IN/subject2template_warp.mif \
        -interp nearest -datatype bit IN/dwi_mask_in_template_space.mif -nthreads $ncpu -force

    else

        for_each -info -force -nthreads ${ncpu_foreach} * : mrtransform IN/dwi_preproced${suffix}_mask.mif -warp IN/subject2template_warp.mif \
        -interp nearest -datatype bit IN/dwi_mask_in_template_space.mif -nthreads $ncpu -force
...
if [ ! -f ../fod_warp.done ]; then
    ...
    for_each -info -force -nthreads ${ncpu_foreach} * : mrtransform IN/wmfod_norm.mif -warp IN/subject2template_warp.mif \
...
if [ ! -f ../fa_adc_warp.done ]; then
    ...
    for_each -info -force -nthreads ${ncpu_foreach} * : mrtransform IN/FA_subj_space.mif -warp IN/subject2template_warp.mif \
      IN/FA_in_template_space.nii.gz -nthreads $ncpu -force
    for_each -info -force -nthreads ${ncpu_foreach} * : mrtransform IN/ADC_subj_space.mif -warp IN/subject2template_warp.mif \
...
if [ ! -f ../fod_segment.done ]; then
    ...
    for_each -info -force -nthreads ${ncpu_foreach} * : fod2fixel -mask ../template/template_mask.mif IN/fod_in_template_space_NOT_REORIENTED.mif \
...
if [ ! -f ../assign_fixels.done ]; then
    ...
    for_each -info -force -nthreads 1 * : fixelcorrespondence IN/fixel_in_template_space/fd.mif \
    ../template/fixel_mask ../template/fd PRE.mif -force
