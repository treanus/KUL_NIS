...
# response function estimation (we compute following algorithms: tournier, tax and dhollander)
    ...
    if [ ! -f response/dhollander_wmfod.mif ]; then
        ...
        dwi2fod msmt_csd dwi_preproced.mif response/wm_response.txt response/dhollander_wmfod.mif response/gm_response.txt response/dhollander_gm.mif \
        response/csf_response.txt response/dhollander_csf.mif -mask dwi_mask.nii.gz -force -nthreads $ncpu 
    ...
    if [ ! -f response/tax.txt ]; then
        kul_e2cl "   Calculating tax dwi2response..." ${log}
        dwi2response tax dwi_preproced.mif response/tax.txt -nthreads $ncpu -force 
    ...
    if [ ! -f response/tax_wmfod.mif ]; then
        ...
        dwi2fod csd dwi_preproced.mif response/tax.txt response/tax_wmfod.mif  \
    ...
    if [ ! -f response/tournier.txt ]; then
        kul_e2cl "   Calculating tournier dwi2response..." ${log}
        dwi2response tax dwi_preproced.mif response/tournier.txt -nthreads $ncpu -force 
    ...
    if [ ! -f response/tax_wmfod.mif ]; then
        ...
        dwi2fod csd dwi_preproced.mif response/tournier.txt response/tournier_wmfod.mif  \
        -mask dwi_mask.nii.gz -force -nthreads $ncpu 
